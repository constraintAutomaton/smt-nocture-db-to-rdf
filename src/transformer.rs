...
pub trait Transformer<'a> {
    ...
    fn to_file(&'a self, path: PathBuf) -> Result<(), Box<dyn std::error::Error>> {
        ...
        file.write_all(DATA_LICENSE.as_bytes())?;
...
impl<'a> Transformer<'a> for BasicFusionRuleTransformer<'a> {
    fn to_rdf(&'a self) -> impl Graph {
        ...
        } in self.rules.iter()
        {
            ...
            let result_term = RaceTransformer::identifier_to_rdf(self.race_namespace, result);
...
impl<'a> DemonTransformer<'a> {
    pub fn new(
        ...
        let cannot_fused_with_basic_rules_iri = vocabulary_namespace.get("specialFusion").unwrap();
...
impl<'a> Transformer<'a> for DemonTransformer<'a> {
    fn to_rdf(&'a self) -> impl Graph {
        ...
        for demon in self.demon.iter() {
            ...
            let basic_fusion_value = if self.cannot_be_fused_with_basic_rules(&demon.name) {
                "true"
            } else {
                "false"
            };
...
const DATA_LICENSE: &'static str = r#"# This data  is made available under the Open Database License: http://opendatacommons.org/licenses/odbl/1.0/.
